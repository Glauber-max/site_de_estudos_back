...
#router for get token and email, this router calls other functions for get create user request and valid, after add in database
@router.post("/validation_account", status_code=201)
async def router_for_validation_token(email: EmailStr, token: str, db: Session = Depends(get_db)):
    responses = compare_redis(email_end=email, code_write=token)
    if not responses:
        raise HTTPException(status_code=400, detail="token incorrect")
    user_controller.function_for_get_user(email=email, db=db)
    return {"message": "Token sent successfully, account activated"}

#this router make a login in project
...
#this function change a password
@router.post("/change_passoword_validation", status_code=200)
...
def verify_token_for_change_password(user: ChangePasswordValidation, token: str, db: Session = Depends(get_db)):
    message = user_controller.verify_change_password(user=user, token=token, db=db)
    return message
