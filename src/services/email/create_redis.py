...
def compare_redis_for_change_password(email: EmailStr, token: str) -> bool:
    try:
        ...
        logger.error(f"failed in read the redis: {e}")
        raise HTTPException(
            status_code=500,
...
def redis_create_user(user: CreateUser, token) -> None:
    try:
        ...
        logger.error(f"failed in create users in redis: {e}")
        raise HTTPException(
            status_code=500,
...
def get_account_after_token_correct(email: EmailStr):
    ...
    try:
        ...
        logger.error(f"json invalid get in redis: {e}")
        raise HTTPException(
            status_code=422,
...
def delete_token_from_redis_after_token(email: EmailStr) -> None:
    try:
        ...
        logger.warning(f"error in delete the files in redis: {e}")
